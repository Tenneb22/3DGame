...
public class MouseInput {
    ...
    public void init() {
        GLFW.glfwSetCursorPosCallback(Main.getWindow().getWindowHandler(), (window, xpos, ypos) -> {
            currentPos.x = xpos;
            currentPos.y = ypos;
        });

        GLFW.glfwSetCursorEnterCallback(Main.getWindow().getWindowHandler(), (window, entered) -> {
            inWindow = entered;
        });


        GLFW.glfwSetMouseButtonCallback(Main.getWindow().getWindowHandler(), (window, button, action, mods) -> {
